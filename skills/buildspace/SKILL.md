...
2. **Secrets**:
   ...
   - npm publishing: `NPM_TOKEN` (required for token-based publishing; optional when using OIDC Trusted Publishing)
...
3. **Permissions**:
   ...
   - OIDC Trusted Publishing needs `id-token: write` on the caller workflow (only when `use-oidc: true`)

## Blacksmith Runners (All Workflows)

All reusable workflows accept a `use-blacksmith` input:

| Input | Type | Required | Default | Description |
|---|---|---|---|---|
| `use-blacksmith` | boolean | No | `false` | Opt in to Blacksmith Linux runners (`blacksmith-4vcpu-ubuntu-2404`). Requires the Blacksmith GitHub App installed on the caller org. Defaults to `ubuntu-latest`. |

When `use-blacksmith: true`, Linux jobs run on `blacksmith-4vcpu-ubuntu-2404` instead of `ubuntu-latest`. macOS jobs are unaffected.
...
| `use-blacksmith` | boolean | No | `false` | Opt in to Blacksmith Linux runners |
...
| `publish-command` | string | No | `npm publish` | Command used to publish (e.g., `npm publish`, `bunx clean-publish`). The `--tag`, `--access`, and `--dry-run` flags are appended automatically. |
| `labels-to-check` | string | No | `["release", "prerelease"]` | PR labels that trigger releases |
| `prerelease` | boolean | No | `false` | Force prerelease |
| `release` | boolean | No | `false` | Force release (bypasses label check) |
| `dry-run` | boolean | No | `false` | Test without actually publishing |
| `use-blacksmith` | boolean | No | `false` | Opt in to Blacksmith Linux runners |
| `use-oidc` | boolean | No | `false` | Opt in to npm OIDC Trusted Publishing. Requires caller `id-token: write` + a configured trusted publisher on npmjs.com. Falls back to `NPM_TOKEN` if OIDC is unavailable. |
...
**OIDC Trusted Publishing (opt-in):**

To enable tokenless npm publishing with provenance:

1. Set `use-oidc: true` in the caller workflow inputs.
2. Add `id-token: write` to the caller's `permissions` block.
3. Configure a [trusted publisher](https://docs.npmjs.com/trusted-publishers) for the package on npmjs.com.
4. Ensure the runner has npm >= 11.5.1.

Callers that don't set `use-oidc` are completely unaffected — they keep `contents: read` least-privilege and publish via `NPM_TOKEN` exactly as before. Keep providing `NPM_TOKEN` as a fallback until OIDC is fully verified.
...
jobs:
  release:
    uses: photon-hq/buildspace/.github/workflows/typescript-service-release.yaml@v1
    permissions:
      contents: write
      pull-requests: read
      id-token: write
...
## npm Publishing Modes

The `publish-npm` block (and workflows that use it) supports two publishing modes:

1. **OIDC Trusted Publishing (preferred for new setups):** When the calling job has `id-token: write`, the action publishes tokenlessly with provenance. Requires a configured [trusted publisher](https://docs.npmjs.com/trusted-publishers) on npmjs.com and npm >= 11.5.1.
2. **Token fallback (default):** If OIDC is unavailable (no `id-token: write`) or fails, the action publishes with `npm-token`. This is the default behavior — keep providing `NPM_TOKEN` so publishing works until OIDC is fully set up.

For single-package TypeScript releases, use the `use-oidc` input on `typescript-service-release` to opt in. Callers that don't set `use-oidc` are completely unaffected.
...
| Block | Path | Purpose |
|---|---|---|
| `publish-npm` | `.github/blocks/publish-npm/action.yaml` | Publish single package to npm (OIDC or token) |
...
| `working-directory` | string | No | `.` | Directory with `package.json` |
...
| `publish-command` | string | No | `npm publish` | Command used to publish. The `--tag`, `--access`, and `--dry-run` flags are appended automatically. |
...
- For OIDC publishing issues, confirm `id-token: write` is in caller permissions, a trusted publisher is configured on npmjs.com, and npm >= 11.5.1 is available.
